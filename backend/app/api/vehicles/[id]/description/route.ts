...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const description = await generateVehicleDescription({ vehicleId: params.id, userId: user.id, additionalNotes: typeof body.additionalNotes === 'string' ? body.additionalNotes : undefined });
