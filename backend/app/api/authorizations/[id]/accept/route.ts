...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const authorization = await acceptAuthorizationByAuthorizedParty({ authorizationId: (await params).id, authorizedUserId: user.id, otpId: p.data.otpId, otp: p.data.otp });
