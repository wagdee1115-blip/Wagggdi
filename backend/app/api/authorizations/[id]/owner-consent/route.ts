...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await verifyAuthorizationOwnerOtp({ authorizationId: id, ownerId: user.id, otpId: parsed.data.otpId, otp: parsed.data.otp });
    ...
    return authorizationErrorResponse(error, 'OWNER_CONSENT_FAILED');
  }
}
