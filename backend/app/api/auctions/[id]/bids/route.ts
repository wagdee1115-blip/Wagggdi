...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!result.replayed && 'auction' in result) {
      await notificationService.sendNotification({
        userId: result.auction.sellerId,
        ...
        priority: 'HIGH', channels: ['IN_APP'], operationId: result.auction.id,
        data: { auctionId: result.auction.id, vehicleId: result.auction.vehicleId },
      });
    }
