...
import { hashPassword, signRegistrationJwt } from '@/lib/auth';
...
export async function POST(req: Request) {
  try {
    ...
    const or: Prisma.UserWhereInput[] = [{ phone: data.phone }];
    ...
    const user = await db.user.create({ data: { fullName:data.fullName, phone:data.phone, email:data.email ?? null, passwordHash:await hashPassword(data.password), role:'USER', status:'PENDING' } });
    const token = await signRegistrationJwt({ sub:user.id, role:user.role, sessionVersion:user.sessionVersion });
