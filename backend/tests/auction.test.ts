...
describe('Auction PostgreSQL verification', () => {
  ...
  dbIt('allows only one winner and creates one sale when finalized', async () => {
    ...
    const vehicle = await db.vehicle.create({ data: { ownerId: seller.id, plateNumber: `W-${suffix.slice(-10)}`, vin: `WIN${suffix}`.slice(0, 17), make: 'TEST', model: 'TEST', year: 2026, price: 1000000, mileage: 0, transmission: 'AUTO', fuelType: 'PETROL', color: 'WHITE', city: 'Sanaa', status: 'ACTIVE', governmentStatus: 'UNKNOWN' } });
